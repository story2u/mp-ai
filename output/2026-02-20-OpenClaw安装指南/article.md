## 19 万 Star 的开源 AI 管家 OpenClaw，四种方法手把手教你安装
...
![OpenClaw 三层架构示意图：聊天平台、Gateway 网关与 LLM+Skills 执行层](illustrations/01-infographic-architecture.png)
...
- 一个 LLM 的 API Key，海外推荐 Anthropic Claude，国内推荐 DeepSeek
- 一个常用聊天平台的账号，Telegram、飞书、钉钉均可
- 一台测试用的机器，别急着在主力设备上折腾
...
脚本会自动检测系统环境、安装 Node.js 依赖、下载 OpenClaw。完成后紧接着运行引导向导：

```bash
openclaw onboard --install-daemon
```

向导会带你走完 8 步配置：选择 LLM 提供商、填入 API Key、配置聊天通道、注册守护进程。macOS 用户从开始安装到发送第一条消息，全程大约 5 分钟。
...
掌控感更强，升级也方便。但有个常见坑：npm 全局路径没加入 PATH，导致终端找不到命令。
...
想在 VPS 上 24 小时跑 OpenClaw，Docker 是最稳妥的选择：
...
容器默认以非 root 用户（node, UID 1000）运行，可以启用沙箱模式隔离 Agent 操作。
...
构建时间较长，需要额外安装 pnpm 包管理器，一般用户没必要走这条路。

### 四种方法横向对比

- 一键脚本：难度最低，2-3 分钟完成，适合新手快速体验，缺点是不透明
- npm 安装：难度较低，掌控感强、易升级，适合 Node.js 开发者，需手动处理 PATH
- Docker 部署：难度中等，安全隔离是最大优势，适合运维人员和 VPS 部署，需要 Docker 基础
- 源码构建：难度最高，完全控制源码，仅适合贡献者和深度定制场景

![四种安装方法对比：一键脚本、npm 安装、Docker 部署与源码构建](illustrations/02-infographic-install-methods.png)
...
不需要科学上网，延迟更低，API 费用大概是海外方案的三分之一到五分之一。
...
![OpenClaw 国内生态方案：汉化版、IM 一键集成版与 DeepSeek 大模型](illustrations/03-infographic-china-ecosystem.png)
...
![OpenClaw 安全部署清单：六条核心安全原则](illustrations/04-infographic-security-checklist.png)

### 遇到问题？先试这几招

- `openclaw: command not found`：npm 全局路径没加入 PATH，参考方法 2 的修复命令
- Node.js 版本报错：`nvm install 22 && nvm use 22`
- 端口 18789 被占用：修改 `~/.openclaw/openclaw.json` 中的端口号
- API Key 报 401：检查 Key 有没有多余空格、是否过期、余额是否充足

实在搞不定？OpenClaw 内置了深度诊断工具：

```bash
openclaw doctor --deep
```

这条命令会检查系统环境、依赖版本、网络连通性、配置文件完整性，帮你定位问题根源。
